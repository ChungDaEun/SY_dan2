...
-- 1. 해당 컬러멩 데이터가 저장되어있을 때 설정된 크기가 아니고 실제 저장된 기준으로보다 작게는 변경 불가능하다
--    타입의 변경을 불가능(문자 <==> 숫자 <==> 날짜)
-- 2. 해당 컬럼에 데이터가 저장되어 있지 않을 경우 크기 상관없이 변경가능, 타입도 변경가능
ALTER TABLE emp13
