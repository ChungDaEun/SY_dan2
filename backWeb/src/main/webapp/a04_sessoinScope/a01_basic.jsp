...
2. 범위 내용
   ...
   3) session 영역 : 하나의 웹브라우저와 서버와 관련된 영역
   4) application 영역 : 하나의 웹 어플리케이션과 관련된 영역
3. 설정 방법
   0) 저장 데이터는 기본 데이터유형, 배열, 객체...
   1) 저장
      범위 객체.setAttribute("저장할 키", 저장할 데이터)
   2) 저장된 내용 호출
      할당할 변수 = 범위객체.getAttribute("저장된 키")
   3) 삭제
      범위 객체.removeAttribute("저장된 키")

4. 각 범위에 따른 처리
   1) pageContext(page)
      페이지 범위까지만 처리된다.
   2) request
      request.getRequestDispatcher(이동페이지).forward(request)
      ...
      # request 범위는 요청값을 가져오거나 request.setAttribute()에 의해 저장한 값을
        forward 처리했을 때 다른 페이지라도 사용할 수 있는 범위다.
   3) session
      서버와 브라우저간에 연결 상태가 있을 때까지 지속되는 범위를 말한다.
      동일한 브라우저로 연결이 있는 상태에서는 a href, location.href로 호출하더라도
      계속 연결이 되어 있기 때문에 설정된 session값을 가져올 수 있다.
      ...
      # 실무적 처리
      1. login ==> 인증되었을 DB id, pass외 이름 권한 포인트 session에 할당
         cf) Dao public Member
         페이지가 권한별로 접근할 때
         ...
         function goMenu(){
         	...
         	if(auth == 'admin'){
         		...
         		alert("해당 페이지는 권한이 없어 접근이 불가능 합니다.")
         	}
         }
      2. 게시판 작성자 입력
         1) session에 의해 작성자가 등록되는 경우
            # 효과1 : 자동등록 된다.
            보이는 화면에 이름(중복 가능한 데이터)가 보인다.
