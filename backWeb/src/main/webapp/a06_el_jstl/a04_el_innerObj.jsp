...
2. 사용법
   ${내장객체.속성}
3. 기본 객체들
   1) scope관련 : pageScope, requestScope, sessionScope, applicateionScope
      ex1) pageContext.setAttribute("name","홍길동")
      ${name} : 위 4개중 하나의 범위 상관없이 출력된다.
      ${pageScope.name} : 출력
      ${requestScope.name} : 출력 안됨
      
      ex2) pageContext.setAttribute("fruit","사과")
      ${fruit} : 위 4개중 하나의 범위 상관없이 출력된다.
      ${pageScope.fruit} : 출력 안됨
      ${requestScope.fruit} : 출력 안됨
      ${sessionScope.fruit} : 출력
   2) pageContext : jsp의 page 기본 객체와 동일
   3) param : 요청 파라미터의 값을 <파라미터 이름, 값>매핑을 저장한 Map 객체
