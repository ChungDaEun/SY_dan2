...
2. 이에, 보다 null 예외를 줄이고 scope에 따른 변수설정을 효과적으로 처리하기위해
   사용하는 언어를 jsp에서 지원하게 되었다.
3. el의 사용과 기본코드
   1) el코드의 기본형식
      el코드는 scope에 의해 설정된 데이터를 가져올때 기본의 getAttribute()로 가져오거나
      요청값을 처리할 때 화면단에서 처리해야하는 데이터를 간편하게 ${@@}형식으로 처리 할 수 있게 하였다.
      - 기본 변수 ${변수명}
      - 객체형데이터 ${변수명.property}
        변수명.get프로퍼터밍()과 동일
...
4. el의 주요 기능
   ...
   3) 수치연산, 관계연산, 논리 연산자 제공
   4) 자바 클래스의 메서드 호출기능 제공
   5) 표현 언어만의 기본 객체 제공
...
<%
	...
	/*
	ex) 물건명, 가격, 갯수를 설정하고 el로 호출하여 아래 화면에서 출력하세요.
	*/
...
%>
    <div class="container mt-3">
    	...
    	<h2>갯수 : ${cnt}</h2>
    	<h2>총계 : ${price * cnt}</h2>
    	<h2>비용 등급 : ${price * cnt >= 10000}</h2>
    	<h2>비용 등급 : ${price * cnt >= 10000? '고비용':'저비용'}</h2>
