...
2. jsp 프로그램을 사용하다보면 null, script(<%%>)코드의 복잡성, session scope 처리 변수 설정에 어려움이 있을 때가 많다.
   효과적인 프로그램을 위해 위와 같은 기능과 반복문 조건문 등 여러가지 설정이 간편하게 처리되는 것이 필요하다.
3. 위와 같은 내용을 개선하여 jstl이라는 표준 태그 라이브러리를 활용해 보다 프로그램적으로 효과적인 처리를 하는 부분이 필요하여 사용한다.
4. 주요 기능
   0) 환경 설정
      WEB-INF\lib\jstl-1.2.jar
      상단에 tag lib 선언
      <%@ taglib prefix = "c" uri = "http://java.sun.com/jsp/jstl/core"%>
      <%@ taglib prefix = "fmt" uri = "http://java.sun/.om/jsp/jstl/fmt"%>
