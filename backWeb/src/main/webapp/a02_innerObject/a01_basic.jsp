...
3. 주요 객체 내용
   ...
   4) session : HTTP 세션 정보를 저장하는 객체이다. 서버와 클라이언트의 연결된 상태를 기준으로 특정변수를 저장 범위로 처리한다.
   ...
   8) exception : 예외객체, 예외 페이지에서만 사용된다.
   # jsp에서 내장 객체 중에서 변수와 scope(시간에 따른 유지범위)
   pageContext : 한 페이지내에서만
   request : 요청한 페이지까지 전달
   session : 서버 VS 브라우저가 연결되어 있을 때까지
   application : 서버가 재가동하는 범위까지

# out 기본 객체
1. jsp 페이지가 생성하는 모든 내용은 out 기본 객체를 통해서 전송할 수 있다.
2. 복잡한 if-else문 사용시, out 기본 객체 사용하면 편리하다.
   1) scriplet, expression의 복잡한 사용보다 특정한 경우 out으로 활용할 때 더 효율적일 때가 많다.
3. 출력 메서드
   1) out.print() : 데이터를 출력만 한다.
   2) out.println() : 데이터를 출력하고 줄 바꿈 처리까지 한다.
   3) out.newLine() : 줄 바꿈 처리를 한다.  
...
<%--
ex) 1~25까지 5X5 테이블을 out이용하여 출력하되 해당 cell을 클릭시, 배경색상이 변경되게 하세요.
1. 기본 tag로 기본틀을 만들다.
2. 반복적으로 처리할 내용 확인 ==> 반복문 처리
3. 반복 step이 규칙으로 다른 내용 ==> if조건문으로 통해서 처리
 --%>
<h2>5X5 테이블</h2> 
<table border>
	<%
