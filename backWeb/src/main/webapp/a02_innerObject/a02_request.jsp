...
5. 다양한 query string 처리 예시
	...
	2) back단(jsp)
		...
		==> backend(서버단결정) ==> front단에서 a02_jsp를 받아서
		==> backend(서버단호출)
	# 결국은 query string 요청값을 client(브라우저)에서 호출하는 것으로 서버단에서 이러한 요청값을 전달하여
	  요청객체(request)로 처리하는 것이다.	    
