...
1. jsp는 WAS에서 다음과 같은 과정으로 처리된다.
   1) 작성시
      jsp : java + html
   2) 변환과정(WAS가 기동되면서 처리)
      jsp ==> servlet JSP로 만들어진다.
      .jsp ==> .java
   3) 컴파일(WAS가 기동되면서 처리)
      .java ==> .class
   4) url로 client에서 호출시
      서블릿에서 .class에 지정된 내용에 따라 response객체에 의해 최종 html로 만들 내용을 전달
   5) 서버의 html ==> 클라이언트에 전달되어 브라우저를 통해 랜더링한 내용을 화면에서 볼 수 있다.
 
# servlet jsp
1. jsp의 원본 java 파일
2. jsp도 식제 java로 전환되어 손쉽게 코드하기 위한 처리 방법 중에 하나이다.
