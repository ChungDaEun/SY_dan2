...
# 수정처리
1. 상세 화면에서 데이터를 확인하고 변경 데이터를 입력하고 수정버튼 클릭시 수정된다.
2. 수정에 필요한 데이터를 받아서 수정되는 dao 처리
3. 수정 요청값을 받아서 수정되는 Servlet controller	
   1) 요청값 처리
   2) 수정 처리되는 메서드 호출
   3) 수정 후 상세 검색되는 메서드 호출
   4) json 데이터 출력
4. 수정 버튼 클릭
   1) 화면단에 수정데이터 입력 후 버튼 클릭시 ajax 호출
   2) success:function(code){}로 수정 후 
      alert('수정 성공') 다시 데이터 로딩 처리
# 삭제 처리	    	
1. 상세화면에서 삭제 버튼 클릭 이벤트 핸들러로 로딩
2. codeDel.do 컨트롤러
   요청값 no
   삭제 Dao처리
   삭제 되었을 때, "Y"
