...
public class A06_CodeDelete extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int ordno = 0;
		if(ordnoS != null && !ordnoS.equals("")) ordno = Integer.parseInt(refnoS);
		
		String val = request.getParameter("val"); if(val == null) val = "";
		
		String noS = request.getParameter("no"); if(noS == null) noS = "";
		int no = 0;
		if(noS != null && !noS.equals("")) no = Integer.parseInt(noS);
		
		// key에 대한 요청값이 있을 때 처리
		Code code = new Code(0,"","",0,0);
		if(no != 0) {
			// 2. 수정처리
			Code del = new Code(no, title, val, refno, ordno);
