...
public class Mart {
	...
	// 구매 갯수와 구매자 처리 메서드
/*
# 1:다관계에서 배열로 다중 객체를 선언할 때는 배열자체가 초기에 크기를 설정하여 처리해야하는 부분이 있고
  한번 크기가 설저오딘 배열을 다시 객체를 생성하기 전까지는 변경이 불가하여 아래와 같이 추할때 로직이 필요하다.
 */
	...
	public void buyProduct(Product prod) {
		if(pArry != null) { //pArry.length nullpointer예외 방지 목적.
			...
			if(bIdx < cnt) { 
				...
				bIdx++;
				if(bIdx == cnt) {
					System.out.println("구매할 종류 갯수(" + cnt+ ")가 완료 되었습니다.");
				}
			}else {
				System.out.println("구매할 종류 갯수(" + cnt+ ")가 더이상 구매불가 합니다.");
			}
		}else {
			checkValid();
		}
	...
	// 외부 사용 필요없어서 private 현재 클래스에서 내부적으로 호출
