...
2. 필요한 경우 조건문이나 프로세스 처리 라인에 의해 throw new 사용자정의 클래스() 형태로 예외를 처리하는 것을 말한다.
3. 기본형식
...
class User01Exception extends Exception{
	public User01Exception() {
		// 생성자는 반드시 상위에 있는 생성자를 호출해야한다.
		super();
	}
	public User01Exception(String errMsg) {
		super("사용자 정의 예러: " + errMsg); //상위 생성자 중 매개변수가 문자열로 된 생성자 호출
	}
