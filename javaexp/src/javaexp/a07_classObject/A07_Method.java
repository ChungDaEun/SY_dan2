...
2. 함수란 특정한 기능을 처리하고 결과값을 리턴하는 기능이 있다면 이것이 객체(class)에 종속되는 경우 메서드라고 한다.
3. 메소드의 핵심기능
   메서드는 매개변수로 데이터를 받아서 전역변수에 저장하기도 하고 전역변수의 데이터와 관계에 있는 로직을 처리하여
...
4. 메소드의 선언
   ...
   2) 구성요소
      - 리턴유형
        이 메소드를 통해서 처리하는 결과값을 리턴을 하는데 메서드 선언의 가장 밑부분에 
      ...
      - 매서드 명(매개변수)
        매개변수는 외부에서 입력한 데이터를 받아서 이 기능 메서드의 지역변수로 활용하여 사용한다.
      	...
      	즉 메서드 오버로딩 규칙이 적용된다.
      - 프로세스 처리
        매개변수를 받아서 전역변수에 할당하거나 특정 조건 처리/반복처리를 하는 등 여러가지 해당 기능을 통해서 
        처리할 로직을 포함하는 기능을 한다.
      - 최종 리턴값
        처음에 선언한 리턴유형에 맞는 최종 리턴값을 해당 메서드를 통해서 처리할 수 있다.
        단, return XXX 형식으로 선언하지 않을 경우도 있는데 이때는 반드시 return유형을 void라고 선언해야한다.
   
 */
		
		Calculator c1 = new Calculator();
		int ret01 = c1.plus(30,25);
		System.out.println("최종리턴값1: " + ret01);
		
		MRExp r1 = new MRExp();
		System.out.println("어떤데이터? " + r1.retStr01());
		...
		MRExp2 login = new MRExp2("dan2","0620");
		System.out.println("Id : " + login.getId());
		System.out.println("Password : " + login.getPass());
		System.out.println("Password : " + login.getIdPass());
		
	}

}
// ex3) class MRExp2 선언하고 필드로 회원아이디(id), 패스워드(pass) 생성자로 초기화 시키고
//      메서드1 getId() ==> 아이디값 가져오기
//      메서드2 getPass() ==> 패스워드값 가져오기
//      메서드3 getIdPass() ==> Id 와 Pass를 @@@/@@@ 형식으로 가져오게 처리를 선언하고
//      main()에서 호출하세요.
class MRExp2{
	String Id;
	String Pass;
...
//		return this.name;
	...
	// ex2) 나이를 리턴하는 메서드를 만들고 main()호출하여 출력하세요.
	int getAge() {
		return age;
	}
	...
	//ex1) 정수 100을 리턴함 메서드 정의하고, 위 메인 메서드에서 바로 출력하는 것과 변수에 넣고 호출하는 것을 만드세요.
	int retInt01() {
		return 100;
	}
