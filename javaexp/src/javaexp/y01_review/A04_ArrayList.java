...
1. 개념 : 데이터를 나열해서 처리하는 것은 배열이라는 개념부터 시작되었다.
         객체형으로 배열의 한계인 동적을 추가/삭제/변경을 지원하는 것으로 선형 구조로 하나씩 추가할 수 있는 구조를 말한다.
         class로 선언된 객체가 만들어 질 때 객체를 generic 형식으로 사용하면 보다 효과적으로 객체형 list 데이터를 처리할 수 있다.
2. 기본 사용
   1) 생성 : ArrayList<타입> 참조변수 = new ArrayList<타입>();
...
3. 처리 순서
   ...
   4) 그 외 ArrayList 메서드를 이용하여 추가, 가져오기, for문 처리등을 한다.
	  slist.add(new @@@@());
	  @@@@ s = slist.get(idx);
	  slist.size() 크기 가져오기..
4. 응용 예제
