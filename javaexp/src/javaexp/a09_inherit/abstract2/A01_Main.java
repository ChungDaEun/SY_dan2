...
/*
# 1:다관계 구조 추상 클래스
1. 기본적으로 추상 클래스는 하나의 상위 클래스(Weapon)를 기준으로 여러가지 형태의 하위 클래스(Rocket, missile...)가 만들어지는 것인데
   이것이 이를 다루는 다른 클래스에서 활요할 때 해당 포함관계를 가진 클래스(Rocket VS ArrayList<Weapon>)가 추상클래스를 상속받는 
   하위 클래스가 기능적으로 추가되어도 메인 클래스(Robot)는 영향을 미치지 않는 형태의 구조로 처리할 때 사용된다.
2. 다음의 구조는 추상 클래스를 활용한 1:다관계의 추상 클래스 구조 중에 하나이다.
   1) 메인 클래스를 Robot을 두고 이 로봇의 여러가지 종류를 필드로 설정하고
   2) 이 Robot이 사용하는 무기를 다양하게 구비할 수 있는 상위 Weapon을 선언하고, 여러가지 무기를 추가 처리할 수 있는 1:다관계 필드를 선언할 수 있다.
      class Robot{
   ...
   3) 추상클래스의 상위 Weapon 선언
      abstract class Weapon{
      	...
      	void adaptWeapon(){ //공통메서드
      		 System.out.println(wname + " 장착하다.");
      	}
      	abstract void attack();
      	// 추상메서드 무기를 사용해서 공격하는 내용
      }
   4) 추가되는 하위 실제 클래스
...
3. 1:다 관계 포함 클래스 만들기
4. 메인메서드에서 처리내용 확인
5. 추상 클래스를 상속받은 실제 클래스 여러가지 형태 추가 및 메인메서드에서 추가 처리 내용 확인
