...
public class A01_StarcraftWorld {
	...
	public static void main(String[] args) {
		// 추상클래스는 혼자 객체를 생성하지 못한다.
		// 추상 클래스의 목적이 하위 클래스의 공통 기능 선언과 다형성(메서드 재정의를 통해)이기 때문
		// 추상 클래스는 다형성 처리할 메서드를 강제성 있게 처리할 목적이기에 실제 클래스가 필요없게 만든다.
		
//		Larba lb = new Larba("드론", " 빨간"); 에러 발생
		Larba lb100 = new Dron(); // 추상 = 하위 실체();
		lb100.move(); // 상속해서 사용할 공통 내용
		lb100.getUnit(); // 상속해서 사용할 공통내용
		lb100.attack(); // 상속해서 사용할 다형성 처리 내용, 재정의가 강제하고 있다.
		
		// TODO Auto-generated method stub
/*
# 추상클래스의 목적
  1) 기능적인 통일성을 가진 상위 클래스를 상속받게 하기 위한 것이다.
     - 기능적 통일성
       공통 필드/메서드 : 그대로 재활용
...
// 추상메서드가 하나라도 있으면 추상 클래스가 되고 드디어 혼자 객체생성을 하지 못한다. Larba li = new Larba(); (X)
// 다형성에 의해서 상속받은 하위객체가 할당할 때만 생성
// Larba l1 = new Dron();
// Larba l2 = new Zergging();
...
class Dron extends Larba{
	...
	@Override
	public void attack() {
		System.out.println(getUnit() + " 채집유닛이라 공격을 못합니다.");
...
class Zerggling extends Larba{
	...
	@Override
	public void attack() {
		System.out.println(getUnit() + " 초기 공격 유닛이고 공격력 5로 빠르게 공격합니다.");	
	}
}
class Hydralisk extends Larba{
	...
	@Override
	public void attack() {
		System.out.println(getUnit() + " 중대형 공격 유닛이고 공격력 15로 원거리 공격을 합니다.");	
	}
	@Override
	public void move() {
		super.move();
