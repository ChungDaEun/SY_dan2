...
2. 기본 객체
   ...
   - java에서는 main()프로그램을 통해서 기본적으로 console창에서 데이터를 키보드를 통해서 입력하여 
     입력된 데이터를 System.in이라는 InputStream을 통해서 관리 할 수 있고
     데이터를 console창에서 출력하는 System.out객체를 통해서 문자열 형식으로 println()메서드를 통해서 출력할 수 있다.
3. InputStream을 통해 입력 데이터를 OutputStream으로 출력 처리하기
   ...
   2) 기본 입력 Stream 객체는 다음과 같이 데이터를 처리한다.
      ...
      is1.read(bytes);
      문자(영문)으로 10자를 입력해서 해당 립력 데이터가 bytes라는 배열에 코드값이 입력된다.
      for(byte b:bytes){
      		(char)b; //입력된 문자 하나씩 가져와서 처리기능
      }
      ==> Scanner는 이러한 Stream객체를 포함하여 데이터를 문자열까지 입력받아서 효율적으로 사용할 수 있게 객체로 만든다.
   3) char 형변환 처리
      char ch01 = (char)data;
   4) OutpuStream os1 = System.out;
      os1.write(data); // 코드값을 char문자로 변환해서 출력
      System.out.println("입력한 문자 출력: " + ch01);
   5) 필수 예외 처리하기 is1.read()에서 io가 발생하기에 컴파일 예외 처리를 해야한다.
 */
		System.out.println("# 기본 입력 문자 처리 #");
		System.out.println("문자 1을 입력하세요: ");
